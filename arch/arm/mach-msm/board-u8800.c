...
static uint32_t audio_icodec_tx_config =
...
void msm_snddev_tx_route_config(void)
{
	...
	rc = gpio_tlmm_config(audio_icodec_tx_config, GPIO_CFG_ENABLE);
	if (rc)
		printk(KERN_ERR "%s: gpio_tlmm_config(%#x)=%d\n",
			__func__, audio_icodec_tx_config, rc);
	else
		gpio_set_value(85, 0);
}
...
void msm_snddev_tx_route_deconfig(void)
{
	...
	rc = gpio_tlmm_config(audio_icodec_tx_config, GPIO_CFG_DISABLE);
	if (rc)
		printk(KERN_ERR "%s: gpio_tlmm_config(%#x)=%d\n",
			__func__, audio_icodec_tx_config, rc);
}
